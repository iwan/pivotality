module Pivotality
  class PivResults
    ...
    def initialize(skip_negative: true)
      @arr = []
    ...
    def to_s
      @arr.map{|e| {
        ...
        req_type: e[:req_type],
        zone_set: e[:zone_set],
        vector:   e[:vector].arr.first(5)
      }}
    end

    def set(operator:, req_type:, zone_set:, vector:)
      if !@skip_negative || vector.any_positive?
          @arr << {operator: operator, req_type: req_type, zone_set: zone_set.sort, vector: vector }
      end
    end
    alias :add :set
